...
const OrderDetails = () => {
    ...
    useEffect(() => {
        getOrderDetails(orderId).then((orderDetailsRes) => {
            ...
            if (orderDetailsRes.status == "error") {
                console.log(orderDetailsRes.error)
                dispatch(toastNotification("Error!", "Something went wrong", "error"));
            } else {
                // console.log(orderDetailsRes); // temp test

                setOrderDetails(orderDetailsRes.order_details);

                setReceiverAddress({
                    otherDetails: orderDetailsRes.order_details.OtherDetailsDes + "  -  " + orderDetailsRes.order_details.receiverVillageName + ", " + orderDetailsRes.order_details.receiverRegionName,
                    phoneCustomer: orderDetailsRes.order_details.ReceiverAddressNum,
                    long: orderDetailsRes.order_details.LongReciver,
                    lat: orderDetailsRes.order_details.LatReciver
                });

                // ---------------------

                checkTripCost(orderId, isTransporter(), orderDetailsRes.order_details.CostLoad, orderDetailsRes.order_details.cityFromId, orderDetailsRes.order_details.cityToId).then((tripCostRes) => {
    ...
    if (typeof orderDetails === "object" && JSON.stringify(orderDetails) !== '{}') {
        ...
        let senderAddress = {
            name: SenderName,
            otherDetails: OtherDetails + "  -  " + senderVillageName + ", " + senderRegionName,
            phoneCustomer: PickupMobile,
            long: LongSender,
            lat: LatSender
        };
                ...
                {orderDetails && <Container fluid className='ps-5 pe-5 pb-5'>
                    ...
                    <Row className="">
                                    ...
                                    <span style={styles.orderStatus}>
                                        {
                                            IsStuckOrder == "1" ? <Badge bg="light" style={{ color: "red" }}>{translate("ORDER_DETAILS.ORDER_STUCK")}</Badge> :
                                                IsReturnedOrder == "1" && IsReturnAccepted == "0" ? <Badge bg="light" style={{ color: "#ff4444" }}>{translate("ORDER_DETAILS.REQUEST_RETURN")}</Badge> :
                                                    IsReturnedOrder == "1" && IsReturnAccepted == "1" && orderStatus == "Delivered" ? <Badge bg="light" style={{ color: "#ff4444" }}>{translate("ORDER_DETAILS.ORDER_RETURNED")}</Badge> :
                                                        IsReturnedOrder == "1" && IsReturnAccepted == "1" ? <Badge bg="light" style={{ color: "#ff4444" }}>{translate("ORDER_DETAILS.ORDER_RETURNING")}</Badge> :
                                                        orderStatus == "Deleted" ? <Badge bg="light" style={{ color: "#ff4444" }}>{orderStatus}</Badge> :
                                                                <Badge bg="light" style={{ color: "#35b09d" }}>{orderStatus}</Badge>
                                        }
                                    </span>
                                </Card.Header>
                                <Card.Body>
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td style={{ textAlign: "right" }}>
                                                        ...
                                                        {orderStatus === 'Waiting for Bids' && <span style={{ float: "right", marginTop: "5px", marginLeft: "10px", cursor: "pointer" }}>
                                                            <FiEdit3 onClick={() => {
                        ...
                        <Col className="mt-5" xl="8">
                            <Row style={{
                                ...
                                {/* {isTransporter() && orderStatus !== "Delivered" && <Col>
                                    {
                                ...
                                {/* {!isTransporter() && orderStatus !== "Delivered" && orderStatus !== "Deleted" && <Col>
                                    {
                                ...
                                <Col>
                                    <Box display="flex" justifyContent="end">
                                        {/* Print order pill */}
                                        {/* (orderStatus !== 'Waiting for Bids') */}
                                        {/* (orderStatus !== 'Waiting for Bids' && isTransporter()) || !isTransporter() */}
                                        {true && <>
                                            <Button
...

                                        {
                                            ...
                                            isTransporter() && <>
                                                ...
                                                {orderStatus === 'Waiting for Bids' && tripCost && <>
                                                ...
                                                {orderStatus !== 'Waiting for Bids' && orderStatus !== "Delivered" && orderStatus !== "Deleted" && <>
                                                ...
                                                {pickupDate && orderStatus !== "Delivered" && currentTransporterId === localStorage.getItem("userId") && <>
...

                                        {
                                            ...
                                            !isTransporter() && <>
                                                ...
                                                {(orderStatus === 'Waiting for Bids' || (orderStatus === 'Order Assigned' && ClientAssignAccepted == 0 && clientAssigneeId != null)) &&
                                                ...
                                                {/* {(orderStatus === 'Waiting for Bids' && orderStatus !== 'Order Assigned') &&
                                                ...
                                                {IsReturnedOrder == 1 && IsReturnAccepted == 1 && orderStatus !== "Delivered" && orderStatus !== "Deleted" && <>
                                                    <ConfirmFinishedReturnedOrder onSuccess={() => { setRefresh(!refresh) }} orderId={orderId} />
                                                </>
                                                }

                                                {/* create new returned order instance */}
                                                {localStorage.getItem("userId") == 41 && orderStatus == "Delivered" && <>
                    ...
                    <Row className="mt-5">
                        ...
                        <Col xl="5">
                            <Card className='shadow h-100 rounded-22'>
                                <Card.Header className="card-header-sm">
                                    {!isTransporter() && (orderStatus === "Waiting for Bids" || orderStatus === "Order Assigned") && DeliveryId == null ? translate("ORDERS.AVAILABLE_OFFERS") : translate("TEMP.LOCATION")}
                                </Card.Header>

                                {!isTransporter() && (orderStatus === "Waiting for Bids" || orderStatus === "Order Assigned") && DeliveryId == null ? <Card.Body className='mt-4' style={{ height: '345.8px', overflowY: 'scroll' }}>
                                    <Table>
                                        <thead>
                                            <tr>
                                        ...
                                        {orderStatus === "Waiting for Bids" && tripCost && tripCost.length !== 0 ? <tbody>
                    ...
                    {
                        !isTransporter() && orderStatus === "Waiting for Bids" && <Row className="mt-5">
                            <Col>
