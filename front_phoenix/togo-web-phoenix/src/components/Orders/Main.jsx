...
const Main = ({ socket, token }) => {
    ...
    const [currentPage, setCurrentPage] = useState(getCurrentPage(history) ?? ("current-orders"));
        ...
        getFunctions(functionType, activePage, transporterFunctionfilter, searchStr)
            .then(resp => {
                ...
                if (resp.data.status == "error") {
                    ...
                    switch (stateKey) {
                        case 'orders':
                            setTotalNumberOfRecs(resp.data.orders.length);
        ...
        */
        ...
        switch (currentPage) {
           /*  case "all-orders":
            ...
            case "current-orders":
                stateKey = "orders";
                functionType = isTransporter() ? "getExclusiveTransporterActiveOrdersByPage" : "getExclusiveClientActiveOrdersByPage";
                break;
            case "previous-orders":
                stateKey = "orders";
                functionType = isTransporter() ? "getExclusiveTransporterFinishedDeletedOrdersByPage" : "getExclusiveClientFinishedDeletedOrdersByPage";
                break;
            case "reviewed-orders":
                stateKey = "orders";
                functionType = isTransporter() ? "getExclusiveTransporterReviewedOrdersByPage" : "getExclusiveClientReviewedOrdersByPage";
                break;
            /* case "transactions":
